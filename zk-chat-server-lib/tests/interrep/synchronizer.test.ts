...
const getAllGroupsMock = async (baseUrl: string): Promise<IInterRepGroupV2[]> => {
    return testGroups
};

const getMembersForGroupMock = async (baseUrl: string, provider: string, name: string): Promise<IGroupMember[]> => {
...
const getRemovedMembersForGroupMock = async (baseUrl: string, provider: string, name: string): Promise<number[]> => {
    if (provider == 'github' && name == 'GOLD')
        return [1, 2];
    else if (provider == 'twitter' && name == 'GOLD')
        return [5];
