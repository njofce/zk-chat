...
class KeyExchangeService {
    ...
    constructor(userService: UserService, keyExchangeBundleRequestStatsService: KeyExchangeBundleRequestStatsService, hasher: Hasher, config: IZKServerConfig) {
    ...
    private async validateMessageRules(zk_proof: RLNFullProof, epoch: string, x_share: string) {
        // Validate epoch
        if (!verifyEpoch(epoch, this.config.epochAllowedDelayThreshold))
            throw "Epoch invalid";
        ...
        // Check spam rules
        if (await this.keyExchangeBundleRequestStatsService.isSpam(zk_proof, epoch, this.config.spamThreshold)) {
