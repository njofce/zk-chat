...
import { generate_encrypted_invite_direct_room, update_direct_room_key } from "rln-client-lib";
...
const ExcangeKeysModal = ({
  ...
  const currentActiveRoom = useAppSelector(state => state.ChatReducer.currentActiveRoom);
  ...
  const handleGenerateRoomKey = async () => {
    try {
      //@ts-ignore
      await generate_encrypted_invite_direct_room(currentActiveRoom?.id).then(invite =>
        setRoomKey(invite)
      );
    } catch (error) {
  ...
  const handleUpdateKey = async () => {
    try {
      //@ts-ignore
      await update_direct_room_key(currentActiveRoom?.id, recipientKey).then(invite => {
        setRoomKey("");
        setRecipientKey("");
